...
public class MappedStatement {
     ...
     * @author shaq
     */
    public List executeQueryForPaginated(StatementScope statementScope, Transaction trans, Object parameterObject, int startRow, int numPerPage) throws SQLException {
        try {
            DefaultRowHandler rowHandler = new DefaultRowHandler();
            executePaginatedQueryWithCallback(statementScope, trans.getConnection(), parameterObject, null, rowHandler, startRow, numPerPage);
    ...
    public Object executeQueryForPaginatedCount(StatementScope statementScope, Transaction trans, Object parameterObject) throws SQLException {
        try {
            Object object = null;

            DefaultRowHandler rowHandler = new DefaultRowHandler();
            executePaginatedCountWithCallback(statementScope, trans.getConnection(), parameterObject, null, rowHandler);
            List list = rowHandler.getList();
            if (list.size() > 1) {
                throw new SQLException("Error: executeQueryForObject returned too many results.");
            } else if (list.size() > 0) {
    ...
    private void executePaginatedCountWithCallback(StatementScope statementScope, Connection conn, Object parameterObject, Object resultObject, RowHandler rowHandler) throws SQLException {
        ...
        try {
            ...
            sqlExecuteQuery(statementScope, conn, sqlString, parameters, SqlExecutor.NO_SKIPPED_RESULTS, SqlExecutor.NO_MAXIMUM_RESULTS, callback);
            errorContext.setMoreInfo("Check the output parameters.");
            if (parameterObject != null) {
                postProcessParameterObject(statementScope, parameterObject, parameters);
            }
    ...
    private String paginatedCountSQL(String sqlString) {
    	 StringBuffer sqlTemplate = new StringBuffer("SELECT COUNT(*) FROM (" + SQL_SECTION + ") entity");
         return StringUtils.replace(sqlTemplate.toString(), SQL_SECTION, sqlString);
	}
     ...
     * @author shaq
     */
    public void executePaginatedQueryWithRowHandler(StatementScope statementScope, Transaction trans, Object parameterObject, RowHandler rowHandler, int startRow, int endRow)
            throws SQLException {
        try {
            executePaginatedQueryWithCallback(statementScope, trans.getConnection(), parameterObject, null, rowHandler, startRow, endRow);
        } catch (TransactionException e) {
            throw new NestedSQLException("Error getting Connection from Transaction.  Cause: " + e, e);
        }
    }
     ...
     * @author shaq
     */
    protected void executePaginatedQueryWithCallback(StatementScope statementScope, Connection conn, Object parameterObject,
                                                     Object resultObject, RowHandler rowHandler,
    ...
    private final static String SQL_SECTION = "#SQL_SECTION#";
     ...
     * @author shaq
     */
    private String paginatedSQL(String sqlString, int startRow, int numPerPage) {
//        StringBuffer sqlTemplate1 = new StringBuffer("SELECT outer.* FROM (SELECT ROWNUM rn, inner.* FROM ( " + SQL_SECTION + "  )  inner) outer WHERE outer.rn >= " + startRow + "AND outer.rn <= " + endRow);
        StringBuffer sqlTemplate = new StringBuffer(SQL_SECTION + "limit "+ startRow +","+numPerPage);
        String str = StringUtils.replace(sqlTemplate.toString(), SQL_SECTION, sqlString);
        return str;
    }
